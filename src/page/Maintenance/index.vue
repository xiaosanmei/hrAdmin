...
<template>
<div class='venue'>
  <div class='title'>维护管理</div>
   <el-button type="primary" round class='button-1' @click='maintenanceSettings'>维护设置</el-button>
  <div class='box-1'>
  ...
  <el-select v-model="venueValue" placeholder="请选择场馆" class='select' >
   ...
   <el-table  :data="tableData" class='table'>
    <el-table-column prop="date"  label="时间" class='title' align='center'></el-table-column>
    ...
    <el-table-column label="维护时间" class='title' align='center'>
        <template slot-scope="scope">
            <div>
        {{scope.row.date}} {{scope.row.maintain_start}}-{{scope.row.maintain_end}}
            </div>
        </template>

    </el-table-column>
    <el-table-column prop="remark" label="备注" class='title' align='center'></el-table-column>
...
export default {
  ...
  methods: {
     ...
     let res = await this.api.maintainVenue(param);
      console.log(res)
      if (res.code == 0) {
